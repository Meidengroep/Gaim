...
namespace Gaim
{
    class Player
    {
        ...
        public Player(Vector2 position, bool newGame, bool knalluh)
        {
            #region ParticleInit
            List<Texture2D>  fireParticles = new List<Texture2D>();
            fireParticles.Add(Global.Game.Content.Load<Texture2D>("Particles/Fire/Fire1"));
            fireParticles.Add(Global.Game.Content.Load<Texture2D>("Particles/Fire/Fire2"));
            fireParticles.Add(Global.Game.Content.Load<Texture2D>("Particles/Fire/Fire3"));
            fireParticles.Add(Global.Game.Content.Load<Texture2D>("Particles/Fire/Fire4"));
            ...
            LoadItems(items);
            LoadShip(properties, items[0]);
            LoadShield(items[1]);
            LoadEngine(items[2]);
            this.currentTexture = straightTexture;
            this.schieten = new Schieten(newGame, knalluh);
        ...
        protected void LoadItems(List<string> items)
        {
        ...
        #region ShipInit
        private void LoadShip(List<string> properties, string shipType)
        {
            ...
            {
                case "Phoenix":
                    this.straightTexture = Global.Game.Content.Load<Texture2D>("Ships/" + shipType + "/straight");
                    this.leftTexture = Global.Game.Content.Load<Texture2D>("Ships/" + shipType + "/left");
                    this.rightTexture = Global.Game.Content.Load<Texture2D>("Ships/" + shipType + "/right");
        ...
        {
            this.velocity = (Input.MousePosition - this.position - this.Origin) * mouseFollowSpeed;
        }
        ...
        public virtual void Update(GameTime gt)
        {
            ...
            schieten.Update(position + new Vector2(currentTexture.Width / 2, 0), gt, knalluh);
