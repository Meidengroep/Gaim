...
namespace Gaim
{
    ...
    abstract class Bullet : ICloneable
    {
        ...
        public virtual Rectangle BoundingBox
        {
            get
            {
                return new Rectangle((int)position.X - (int)origin.X, (int)position.Y - (int)origin.Y, 
    ...
    class Flamethrower3 : Bullet
    {
        ...
        protected override void Behaviour(GameTime gt)
        {
            orbit.X = 50 * (float)Math.Sin(((double)gt.TotalGameTime.TotalMilliseconds) * 2);
            orbit.Y = 50 * (float)Math.Cos(((double)gt.TotalGameTime.TotalMilliseconds) * 2);
        }

        public override void Update(GameTime gt)
        {
            this.playerMid = new Vector2(0, Global.Player.Middle.Y);
            this.position = Global.Player.MiddlePosition - playerMid;
            this.Behaviour(gt);
            if (particleEngine != null)
            {
    ...
    class Laser : Bullet
    {
        ...
        protected override void Behaviour(GameTime gt)
        {
            try { position = Global.Player.MiddlePosition; }
            catch { position = new Vector2(300, 750); }
        }
    }
