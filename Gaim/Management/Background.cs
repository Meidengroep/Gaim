...
namespace Gaim
{
    class Background
    {
        ...
        private void LoadBackgrounds(string level)
        {
            for (int i = 0; i < backgrounds.Length; i++)
            {
                backgrounds[i] = Global.Game.Content.Load<Texture2D>("Backgrounds/L" + level + "/" + i.ToString());
                if (i - 1 >= 0)
        ...
        public void Update(GameTime gameTime)
        {
            ...
            {
                if (bgPositions[n] >= Global.ViewPortHeight)
                {
